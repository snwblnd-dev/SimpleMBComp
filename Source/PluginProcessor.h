...
2) create parameters to contrtol where split happens
3) prove that splitting into three bands produces no artifacts
4) create audio parameters for the three compressor bands
    these need to live on each band instance
5) add two remaining compressors
6) add ability to mute, solo, and bypass individual compressors
7) add input/output makeup gain to offset changes in level from dsp
...
namespace Params {
    ...
    //does not get made until called
